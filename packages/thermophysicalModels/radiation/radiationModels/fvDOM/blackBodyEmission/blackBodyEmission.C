...
) const
{
    ...
    else
    {
        ...
        //Info << " Band init: " << band[0] << endl;
        //Info << " Band end: " << band[1] << endl;
        //Info << " Here 2" << endl;
        //Info << " energy frac: " << (T1-T2) << endl;


        const volScalarField::Boundary& TBf =
           T.boundaryField();

        volScalarField::Boundary& Ebf =
           Eb.ref().boundaryFieldRef();
...
{
    enFracLambda = fracDeltaLambdaT(T_, band);
}

Foam::tmp<Foam::volScalarField>
Foam::radiation::blackBodyEmission::fracDeltaLambdaT
(
    const volScalarField& T,
    const Vector2D<scalar>& band
) const
{
    tmp<volScalarField> frac
    (
        new volScalarField
        (
            IOobject
    ...
    {
        scalarField& fracif   = frac.ref().primitiveFieldRef();
        const scalarField& TCells = T.primitiveField();
        ...
        volScalarField::Boundary& fracf =
           frac.ref().boundaryFieldRef();
